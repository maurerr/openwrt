...
# OpenWrt 22.03+ uses firewall4/nftables. Ensure the legacy-named
# firewall.user script is explicitly marked fw4-compatible before use.
if [ -x /sbin/fw4 ]; then
	[ -f /etc/firewall.user ] || touch /etc/firewall.user
	if ! uci -q get firewall.rooter_ttl >/dev/null 2>&1; then
		uci set firewall.rooter_ttl=include
		uci set firewall.rooter_ttl.enabled='1'
		uci set firewall.rooter_ttl.type='script'
		uci set firewall.rooter_ttl.path='/etc/firewall.user'
		uci set firewall.rooter_ttl.fw4_compatible='1'
		uci commit firewall
	fi
fi
...
if [ "$TTL" = "1" ]; then
	...
	if [ ! -z "$vs" ]; then
		...
		r2="nft add rule inet fw4 mangle_prerouting iifname $IFACE ip ttl set $TTL"
		...
		r5="nft add rule inet fw4 mangle_prerouting iifname $IFACE ip6 hoplimit set $HL"
