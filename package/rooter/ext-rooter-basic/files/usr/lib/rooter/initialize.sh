...
firstboot() {
	...
	if [ "$mxchge" != "1" ]; then
		if [ -e /etc/config/mwan3 ]; then
			uci set maxmodem.maxmodem.maxmodem=4
		else
...
# OpenWrt owns /etc/openwrt_release and /usr/lib/os-release.
# Keep ROOter's version in /etc/revision without replacing OpenWrt metadata.
...
while [ $COUNTER -le $MODCNT ]; do
	...
	uci set network.wan$COUNTER'_6'.proto=dhcpv6
	uci set network.wan$COUNTER'_6'.reqaddress=try
	uci set network.wan$COUNTER'_6'.reqprefix=auto
